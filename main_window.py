    ...
    def _find_routes(self):
        ...
        try:
            ...
            for idx, (path, travel_time, distance) in enumerate(route_infos):
                ...
                avg_speed = distance / (travel_time / 60)  # km/h
                # Calculate number of intersections
                intersections = len(path) - 2  # excluding start and end points
                # Get major landmarks along the route
                landmarks = []
                for site_id in path:
                    site_row = self.df[self.df['site_id'].astype(str) == str(site_id)].iloc[0]
                    if 'Location' in site_row:
                        landmarks.append(site_row['Location'])
                ...
                <div style='background:#23272b; border-radius:18px; margin-bottom:22px; padding:28px 32px; color:#fff; font-size:22px;'>
                  ...
                  <div style='margin-top:12px;'>
                    <div style='font-size:21px; color:#7ecfff;'>
                      <b>Estimated Time:</b> {travel_time:.1f} minutes
                    </div>
                    <div style='font-size:21px; color:#f7c873;'>
                      <b>Total Distance:</b> {distance:.2f} km
                    </div>
                    <div style='font-size:21px; color:#98c379;'>
                      <b>Average Speed:</b> {avg_speed:.1f} km/h
                    </div>
                    <div style='font-size:21px; color:#e06c75;'>
                      <b>Intersections:</b> {intersections}
                    </div>
                    <div style='font-size:18px; color:#888; margin-top:8px;'>
                      <b>Via Sites:</b> {' → '.join(str(s) for s in path)}
                    </div>
                    <div style='font-size:18px; color:#888; margin-top:8px;'>
                      <b>Major Landmarks:</b> {' → '.join(landmarks) if landmarks else 'No major landmarks'}
                    </div>
                  </div>
                </div>
                """
            ...
            # Add route comparison summary
            if len(route_infos) > 1:
                fastest_route = min(route_infos, key=lambda x: x[1])
                shortest_route = min(route_infos, key=lambda x: x[2])
                summary_html = f"""
                <div style='background:#2c313a; border-radius:18px; margin:22px 0; padding:20px 32px; color:#fff;'>
                    <div style='font-size:24px; color:#61afef; margin-bottom:12px;'>
                        <b>Route Comparison Summary</b>
                    </div>
                    <div style='font-size:18px; color:#abb2bf;'>
                        <div style='margin:8px 0;'>
                            <b>Fastest Route:</b> Route {route_infos.index(fastest_route)+1}
                            <span style='color:#98c379;'>（Saves {max(x[1] for x in route_infos) - fastest_route[1]:.1f} minutes）</span>
                        </div>
                        <div style='margin:8px 0;'>
                            <b>Shortest Route:</b> Route {route_infos.index(shortest_route)+1}
                            <span style='color:#e5c07b;'>（Saves {max(x[2] for x in route_infos) - shortest_route[2]:.2f} km）</span>
                        </div>
                        <div style='margin:8px 0;'>
                            <b>Number of Routes:</b> {len(route_infos)} available routes
                        </div>
                    </div>
                </div>
                """
